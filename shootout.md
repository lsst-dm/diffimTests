...
![](shootout/fig5.png)
We see that while performance degrades for both algorithms at high densities, the rate of false positives for ZOGY surpasses that of A&L above stellar densities of $\sim> 1,000$ ($\sim > 10^6$ per sq. deg.). We note that for these simulations we added random "atmospheric scintillation" at the scale of 0.05 pixels (roughly 0.1 mas at LSST pixel scale). This may be an overestimation and could be the reason for A&L degradation at high densities. While this may be an overestimation of the true expected effect of scintillation on astrometry for LSST, it is possible that atmospheric effects plus other systematics could lead to this type of registration error. This will need to be evaluated on real images to understand better.