...
class DiffimCorrectionTest(lsst.utils.tests.TestCase):
    ...
    def _setUpSourcelessImages(self, svar=0.04, tvar=0.04):
        """!Generate a fake aligned template and science image.
        """
        ...
        self.im1ex, self.im2ex = makeFakeImages(
            svar=self.svar, tvar=self.tvar,
            psf1=self.psf1_sigma, psf2=self.psf2_sigma,
            n_sources=0, seed=22, varSourceChange=0,
            psf_yvary_factor=0, verbose=False)
    ...
    def _makeAndTestUncorrectedDiffim(self):
        ...
        varpx = self._computePixelVariance(diffExp.getMaskedImage())
        self.assertLess(varpx, expected_var)

        # Uncorrected diffim exposure - variance plane is wrong (too low)
        mn = self._computeVarianceMean(diffExp.getMaskedImage())
        self.assertLess(mn, expected_var)
        if verbose:
            print('Uncorrected variance (pixels, mean var plane):', varpx, mn)
    ...
    def _testDecorrelation(self, expected_var, corrected_diffExp):
        ...
        if verbose:
            print('Corrected variance (pixels, mean var plane):', var, mn)
