...
def getCutoutAndPsf(calexp, srcRec):
    ...
    R : `lsst.pipe.base.Struct`
        - ``cutExp`` : PSF size subexposure cutout.
        - ``psfIm`` : The PSF image calculated at the centroid position.
        - ``bbox`` : bounding box of the calculated PSF image
    """
    psfCenter = geom.Point2D(srcRec['base_SdssCentroid_x'],
                             srcRec['base_SdssCentroid_y'])
    psf = calexp.getPsf()
