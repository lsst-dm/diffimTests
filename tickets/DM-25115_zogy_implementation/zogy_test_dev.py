...
class ZogyTest(lsst.utils.tests.TestCase):
    ...
    def _setUpImagesFlexible(self, svar=100., tvar=100., varyPsf=0., n_sources=0, varSourceChange=0.1):
        ...
        self.im1ex, self.im2ex \
            = makeFakeImages(size=(256, 256), svar=self.svar, tvar=self.tvar,
                             ...
                             n_sources=n_sources, psf_yvary_factor=varyPsf, varSourceChange=varSourceChange,
                             seed=seed, verbose=False)
        # Create an array corresponding to the "expected" subtraction (noise only)
        rng = np.random.default_rng(seed)
        self.expectedSubtraction = rng.normal(scale=np.sqrt(svar), size=self.im1ex.getDimensions())
        self.expectedSubtraction -= rng.normal(scale=np.sqrt(tvar), size=self.im2ex.getDimensions())
